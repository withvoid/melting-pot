  ...
  {() => {
    ...
    return (
      <div {...bind} style={styles.wrapper}>
        <p>
          Touch on the white background to change the status, this will
          work for only touch enabled devices.
        </p>
        <p>Status: <span style={styles.emphasis}>{String(touched)}</span></p>
      </div>
    )
  }}
</Playground>
